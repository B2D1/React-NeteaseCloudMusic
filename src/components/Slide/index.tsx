        ...
        >
          {slide.banners.map((v: any, i: number) => (
            <div
              ...
              onTouchMove={evt => {
                slide.swipeBannerMove(evt, this.slideRef.current);
              }}
              onTouchEnd={evt => {
                slide.swipeBannerEnd(evt, this.slideRef.current);
              }}
            >
              <img src={v.imageUrl} />
              <span
                className={`slide-tag ${
                  v.titleColor === 'blue' ? 'tag-blue' : 'tag-red '
                }
                `}
              >
                {v.typeTitle}
          ...
          <ul>
            {slide.banners.map((v: any, i: number) =>
              i && i !== slide.Len - 1 ? (
                ...
                (slide.curIndex === 0 && i === slide.Len - 2) ? (
